...
const AllCategory = (props) => {
  ...
  const editCategory = (cId, type, name) => {
    if (type) {
      dispatch({
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center p-8">
        <svg
          class="w-12 h-12 animate-spin text-gray-400"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
  ...
  return (
    <Fragment>
      <div className="col-span-1 overflow-auto bg-black shadow-lg p-4">
        <table className="table-auto border w-full my-2 text-white">
          <thead>
            <tr>
            ...
            {categories && categories.length > 0 ? (
              categories.map((item, key) => {
                return (
                  <CategoryTable
                    category={item}
                    editCat={(cId, type, name) =>
                      editCategory(cId, type, name)
                    }
